...
__export(src_exports, {
  ...
  useSignalSelector: () => useSignalSelector,
  useStore: () => useStore
});
...
var import_alien_signals = require("alien-signals");
var alienEngine = {
  signal(value) {
    const s = (0, import_alien_signals.signal)(value);
    return {
      get: () => s(),
      set: (v) => s(v)
    };
  ...
  computed(fn) {
    const c = (0, import_alien_signals.computed)(fn);
    return { get: () => c() };
  },
  effect(fn) {
    return (0, import_alien_signals.effect)(fn);
  },
  batch(fn) {
    (0, import_alien_signals.startBatch)();
    try {
      fn();
    } finally {
      (0, import_alien_signals.endBatch)();
    }
  },
  createScope() {
    let dispose;
    return {
      run(fn) {
        let result;
        dispose = (0, import_alien_signals.effectScope)(() => {
          result = fn();
        });
        return result;
      },
