...
import { assert } from './utils'; // The global revision clock. Every time state changes, the clock increments.

export let $REVISION = 0; // The current dependency tracker. Whenever we compute a cache, we create a Set
...
export function setValue(storage, value) {
  assert(storage instanceof Cell, 'setValue must be passed a tracked store created with `createStorage`.');
  storage.value = storage._lastValue = value;
}
export function createCell(initialValue, isEqual = tripleEq, name) {
  return new Cell(initialValue, isEqual, name);
}
export function createCache(fn) {
  assert(typeof fn === 'function', 'the first parameter to `createCache` must be a function');
